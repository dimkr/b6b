...
int main()
{
	...
	assert(b6b_call_copy(&interp, "{$str.index}", 12) == B6B_ERR);
	b6b_interp_destroy(&interp);

	assert(b6b_interp_new_argv(&interp, 0, NULL, B6B_OPT_TRACE));
	assert(b6b_call_copy(&interp, "{$str.index abc 0}", 18) == B6B_OK);
	...
	assert(b6b_interp_new_argv(&interp, 0, NULL, B6B_OPT_TRACE));
	assert(b6b_call_copy(&interp, "{$str.index abc 2}", 18) == B6B_OK);
	assert(b6b_as_str(interp.fg->_));
	assert(interp.fg->_->slen == 1);
	...
	assert(b6b_interp_new_argv(&interp, 0, NULL, B6B_OPT_TRACE));
	assert(b6b_call_copy(&interp, "{$str.index abc 3}", 18) == B6B_ERR);
	b6b_interp_destroy(&interp);
	...
	assert(b6b_interp_new_argv(&interp, 0, NULL, B6B_OPT_TRACE));
	assert(b6b_call_copy(&interp, "{$str.index {} 0}", 17) == B6B_ERR);
	b6b_interp_destroy(&interp);
